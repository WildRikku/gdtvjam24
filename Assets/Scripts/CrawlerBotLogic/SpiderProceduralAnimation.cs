...
public class SpiderProceduralAnimation : MonoBehaviour {
    ...
    private void FixedUpdate() {
        ...
        if (lastBodyPos != (Vector2)transform.position) {
            resetStep = false;
        }
        ...
        if (nbLegs > 1 && bodyOrientation) {
            ...
            transform.rotation = Quaternion.LookRotation(transform.parent.forward, constrainedUp);
            lastBodyUp = transform.up;
        }
    }

    private Vector3 ConstrainTilt(Vector3 up, float maxTiltAngle) {
