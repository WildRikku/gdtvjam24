...
namespace DTerrain {
    public class PaintableLayer<T> : MonoBehaviour, ILayer<T> where T : PaintableChunk {
        [field: SerializeField] public int ChunkCountX { get; set; }
        [field: SerializeField] public int ChunkCountY { get; set; }
        ...
        [field: SerializeField] public Texture2D OriginalTexture { get; set; }

        [field: SerializeField] public int PPU { get; protected set; }

        [SerializeField]
        protected FilterMode filterMode;

        [field: SerializeField] public int SortingLayerID { get; set; }
        ...
        public virtual void SpawnChunks() {
            ...
            for (int i = 0; i < ChunkCountX; i++) {
                for (int j = 0; j < ChunkCountY; j++) {
                    ...
                    piece.SetPixels(0, 0, chunkSizeX, chunkSizeY,
                        OriginalTexture.GetPixels(i * chunkSizeX, j * chunkSizeY, chunkSizeX, chunkSizeY));
                    piece.Apply();
        ...
        public void Paint(PaintingParameters paintingParameters) {
            ...
            foreach (Range r in paintingParameters.Shape.Ranges) {
                PaintColumn(paintingParameters.Position.x + k, paintingParameters.Position.y + r.Min, r,
                    paintingParameters);
                k++;
            }
        }
        ...
        private void PaintColumn(int x, int y, Range r, PaintingParameters pp) {
            ...
            while (true) {
                if (cid >= 0 && cid < Chunks.Count && k + ychunk < ChunkCountY && (k - 1) * chunkSizeY <= height) {
                    Chunks[cid].Paint(new(posInChunkX, posInChunkY - k * chunkSizeY, 1, r.Length + 1), pp);
                    cid++;
                    k++;
                }
                else {
