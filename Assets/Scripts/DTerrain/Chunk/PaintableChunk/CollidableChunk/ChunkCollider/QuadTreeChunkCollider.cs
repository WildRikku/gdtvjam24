...
namespace DTerrain {
    public class QuadTreeChunkCollider : MonoBehaviour, IChunkCollider {
        ...
        public void UpdateColliders(List<Column> pixelData, ITextureSource textureSource) {
            ...
            Vector2Int textureSize = new(textureSource.Texture.width, textureSource.Texture.height);

            if (rects != null) {
                rects.Clear();
            }

            List<BoxCollider2D> colls = new(gameObject.GetComponents<BoxCollider2D>());
            ...
            foreach (Rect r in rects) {
                ...
                Vector2 rColliderOffset = new(r.x + r.size.x / 2, r.y + r.size.y / 2f);
        ...
        private void QuadTreeToRect(List<Column> chunk, int x, int y, int sizeX, int sizeY) {
            ...
            if (hasAnyGround && !hasAnyAir) {
                rects.Add(new(x / PPU, y / PPU, sizeX / PPU, sizeY / PPU));
            }
