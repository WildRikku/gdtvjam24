...
namespace DTerrain {
    ...
    public class CollidableChunk : PaintableChunk {
        ...
        private void DeleteFromColumns(RectInt rect) {
            RectInt common;
            rect.Intersects(new(0, 0, TextureSource.Texture.width, TextureSource.Texture.height), out common);

            for (int i = 0; i < common.width; i++) {
                colliderChanged = columns[common.x + i].DelRange(new(common.y - 1, common.y + common.height)) ||
                                  colliderChanged;
            }
        }
        ...
        private void AddToColumns(RectInt rect) {
            ...
            for (int i = 0; i < common.width; i++) {
                colliderChanged = columns[common.x + i].SumRange(new(common.y, common.y + common.height - 1)) ||
                                  colliderChanged;
            }
        }
