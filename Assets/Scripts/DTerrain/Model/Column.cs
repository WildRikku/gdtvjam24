...
namespace DTerrain {
    ...
    public class Column {
        ...
        public void SingleDelRange(int pos) {
            Range r = Within(pos);
            if (r != null) {
                Range r1 = new(r.Min, pos - 1);
                Range r2 = new(pos + 1, r.Max);
                if (r1.Length > 0) {
                    AddRange(r1);
                }
        ...
        public bool DelRange(Range delr) {
            ...
            for (int i = 0; i < Ranges.Count; i++) {
                if (Ranges[i].Min < a && Ranges[i].Max > b) ///0---a-----b----1
                {
                    changed = true;
                    if (Mathf.Abs(a - b) == 0) {
                        Ranges.Add(new(Ranges[i].Min, a - 1));
                        Ranges.Add(new(b + 1, Ranges[i].Max));
                        Ranges.Remove(Ranges[i]);
                        break;
                    }

                    Ranges.Add(new(Ranges[i].Min, a));
                    Ranges.Add(new(b, Ranges[i].Max));
                    Ranges.Remove(Ranges[i]);
                    break;
                }
